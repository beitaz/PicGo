<template>
  <div id="setting-page">
        ...
        >
          ...
          <el-submenu index="sub-menu">
            ...
            <template v-for="item in picBed">
              <el-menu-item v-if="item.visible" :index="item.type" :key="item.type">
    ...
    <el-dialog title="修改快捷键" :visible.sync="keyBindingVisible">
    ...
    <el-dialog title="自定义链接格式" :visible.sync="customLinkVisible">
      <el-form label-position="top" :model="customLink" ref="customLink" :rules="rules">
        <el-form-item label="用占位符$url来表示url的位置" prop="value">
          <el-input class="align-center" v-model="customLink.value" :autofocus="true"></el-input>
        </el-form-item>
      </el-form>
      <div>如[]($url)</div>
      <span slot="footer">
  ...
  methods: {
    ...
    confirmCustomLink () {
      this.$refs.customLink.validate(valid => {
        if (valid) {
          db.read()
            .set('customLink', this.customLink.value)
            .write()
          this.customLinkVisible = false
